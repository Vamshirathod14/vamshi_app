...
const GREETINGS = {
  morning: {
    ...
    body: "A new day, a fresh start. Open Liv and stay on top of today's plan and spending.",
  },
  afternoon: {
    hour: 14,
...
export async function scanDueNotifications(now = new Date()) {
  ...
  for (const r of dueReminders) {
    ...
    if (r.repeat === "none" && r.alarmMode !== false) {
      ...
      const res = await deliverScheduledNotification({
        ...
        title: r.title,
        body: r.description || "Your Liv reminder is due.",
        url: "/reminders",
  ...
  for (const t of dueTasks) {
    ...
    const res = await deliverScheduledNotification({
      ...
      body: t.description || "Your Liv task reminder is due.",
      url: "/tasks",
      createInAppNotification: settings.inApp,
